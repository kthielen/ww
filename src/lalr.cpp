...
prec::prec() : level(0), asc(assoc::non) {
}

prec::prec(nat level, assoc::pref asc) : level(level), asc(asc) {
}
...
lrstate_table lalrTable(const grammar& g, terminal* s, const precedence& px) {
...
lrstate_table lalrTable(const parserdef& p, const precedence& px) {
	lrstate_table r;
